...
def test_display_help(capsys):
    ...
    captured = capsys.readouterr() # capsys is a pytest fixture that allows us to capture output to stdout and stderr. The readouterr() method returns a tuple containing the captured output, which we can then assert against our expected output. stdout is the standard output stream where print statements write to, and stderr is the standard error stream where error messages are typically written. By using capsys, we can verify that the display_help function is producing the correct output when called.
    expected_output = """
...
def test_calculator_unsupported_operation(monkeypatch, capsys):
    ...
    # Assert
    captured = capsys.readouterr()
    assert "Unsupported calculation type: 'circumference'." in captured.out
