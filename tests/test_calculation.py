...
def test_equality_with_non_calculation():
    """Test __eq__ method with non-Calculation objects (line 222)."""
    calc = Calculation(operation="Addition", operand1=Decimal("2"), operand2=Decimal("3"))
    # Test equality with different types should return NotImplemented
    result = calc.__eq__("not a calculation")
    assert result is NotImplemented
    result = calc.__eq__(42)
    assert result is NotImplemented
    result = calc.__eq__(None)
    assert result is NotImplemented


def test_calculation_failure_exception():
    """Test exception handling in calculation (line 81).
    
    This tests the specific exception handling in the _calculate method
    where InvalidOperation, ValueError, or ArithmeticError are caught 
    and re-raised as OperationError.
    """
    from decimal import InvalidOperation
    
    with patch('builtins.pow', side_effect=InvalidOperation("Forced arithmetic failure")):
        with pytest.raises(OperationError, match="Calculation failed: Forced arithmetic failure"):
            Calculation(operation="Power", operand1=Decimal("2"), operand2=Decimal("3"))
